...
Addon PHP para WHMCS que expõe 93 tools via Model Context Protocol.
...
rg -o "name: '[a-z_0-9]+'" src/Tools/*.php | wc -l  # 93 tools total (rg -o '#\[McpTool' conta mais: descontar os comentários "sem #[McpTool]" de QuoteTools.php e ChipTools.php)
...
- `src/Mcp/` — `ServerAdapterInterface` + `McpSdkAdapter`: builder do SDK com discovery por atributo cacheada em `data/cache/mcp_elements.json` via `FileElementCache` (PSR-16, `unserialize` com allowlist fechada de 4 classes + pré-varredura, arquivo 0600); sessões `SecureFileSessionStore` (dir 0700, arquivos 0600, chmod fail-closed) em `data/sessions/` TTL 1h GC 1/20 apenas para o transporte legado; `SessionLock` (um arquivo flock POR SESSÃO em `data/session-locks/`, nome = `sess-<sha256 do id>.lock`, GC 1/20 de arquivos > 1h não segurados, 503+Retry-After se indisponível) serializa somente requests legados com `Mcp-Session-Id`; `RuntimeDirs::provision()` cria/endurece os 3 dirs na ativação e no upgrade; `StreamableHttpTransport` com `middleware: []` e `maxBodyBytes` 1 MiB; SDK 0.8 classifica a era internamente e aplica `ProtocolVersionMiddleware` no handshake legado ou `StandardHeaderValidator` no fluxo moderno; `setProtocolVersion(2025-11-25)` + `setModernVersions([2026-07-28])` explícitos
...
- `src/Tools/*.php` — 16 tool classes, 93 tools: Client(12), ProjectManager(9), Order(7), Quote(7), Chip(6), System(6), Ticket(5), Domain(5), Billing(5), CRM(4), Translation(4), TranslationCatalog(5), TranslationCatalogExtras(6), TranslationContent(8), SupportInfo(3), Service(1)
...
- TODAS as classes de efeito colateral ficam bloqueadas por padrão — WRITE inclusive (`gateEnabled()` passa `default: false` para todas). Cada uma exige opt-in próprio (`nt_mcp_enable_write`, `nt_mcp_enable_destructive`, …), com valor canônico `'1'` (`'on'`/`'true'`/`'yes'` NÃO são aceitos por `ConfigFlag`). Gates e allowlists de alvo têm UI: painel "Gates de Efeito Colateral" no dashboard do addon (`GateConfigAction` valida/canonicaliza, `AdminController` grava, alteração auditada como `MCP ADMIN GATE CONFIG CHANGED`).
...
- Fase 4 (última) do domínio de tradução — base de conhecimento e anúncio (`TranslationContentTools`, 8 tools) — usa modelo de linha-filha (`parentid`/`catid` + `language`), igual `tblemailtemplates`, diferente de `tbldynamic_translations` (Fases 2/3): a linha original tem `parentid=0`/`catid=0` e `language=''`; a variante é uma linha filha. `KnowledgebaseRepository` é a ÚNICA classe que toca `tblknowledgebase` E `tblknowledgebasecats`; `AnnouncementRepository` é a ÚNICA classe que toca `tblannouncements`. DESVIO do plano original: `whmcs_translation_announcement_set` usa Capsule direto (não LocalAPI) — `AddAnnouncement`/`UpdateAnnouncement` não expõem `parentid`/`language`, então não criam variante de idioma. Categoria de KB não tem tool de "get": nome/descrição cabem na listagem, mesmo corte de `product_group`.
...
- Middleware do SDK: a lista externa do transport fica vazia. O SDK 0.8 aplica internamente `ProtocolVersionMiddleware` no handshake legado e `StandardHeaderValidator` nas requests modernas. `CorsMiddleware`/`DnsRebinding` seguem desligados de propósito — CORS/IP/TLS são nossos, em mcp.php. O CORS permite `MCP-Protocol-Version`, `MCP-Session-Id`, `MCP-Method` e `MCP-Name`; perfil do mcp.php = `POST, DELETE, OPTIONS` (DELETE encerra sessão legada); oauth.php continua `POST, OPTIONS`
- Lock por sessão (`SessionLock`): somente requests legadas que trazem `Mcp-Session-Id` usam o lock. O `FileSessionStore` do SDK faz read-modify-write sem lock — requests concorrentes com o mesmo ID perdiam resposta e cruzavam respostas entre clientes (reproduzido). Lock segurado até depois do `emit`; initialize e requests modernas stateless não travam. UM arquivo por sessão (era 64 faixas `crc32 % 64`, que serializava sessões DIFERENTES caídas na mesma faixa — com o lock segurado pelo request inteiro, um cliente parado derrubava outro com 503)
