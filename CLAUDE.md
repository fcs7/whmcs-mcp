...
- **`mcp/sdk` pinado em 0.8.0** (pre-1.0) — esta versão introduz o transporte dual-era; qualquer upgrade continua exigindo branch dedicada e bateria legado+moderno
...
- **Versões de protocolo**: SDK 0.8.0 mantém o handshake legado com `2025-11-25` e sessão obrigatória após `initialize`; a era moderna é pinada em `2026-07-28`, usa headers `MCP-Method`/`MCP-Name` e é stateless (`server/discover`, `tools/list`, `tools/call` sem initialize). Header/body incompatíveis falham fechados
